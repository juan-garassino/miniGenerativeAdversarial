import time
from miniGan.agent.utils import getCreds, makeApiCall
...
def createMediaObject(params):
    ...
    url = params['endpoint_base'] + params[
        'instagram_account_id'] + '/media'  # endpoint url
    ...
    if 'IMAGE' == params['media_type']:  # posting image
        ...
        endpointParams['media_type'] = params[
            'media_type']  # specify media type
...
def publishMedia(mediaObjectId, params):
    ...
    url = params['endpoint_base'] + params[
        'instagram_account_id'] + '/media_publish'  # endpoint url
...
def getContentPublishingLimit(params):
    ...
    url = params['endpoint_base'] + params[
        'instagram_account_id'] + '/content_publishing_limit'  # endpoint url
...
params[
    'media_url'] = 'https://justinstolpe.com/sandbox/ig_publish_content_img.png'  # url on public server for the post
params[
...
imageMediaObjectResponse = createMediaObject(
    params)  # create a media object through the api

print(imageMediaObjectResponse)

imageMediaObjectId = imageMediaObjectResponse['json_data'][
    'id']  # id of the media object that was created
...
while imageMediaStatusCode != 'FINISHED':  # keep checking until the object status is finished
    imageMediaObjectStatusResponse = getMediaObjectStatus(
        imageMediaObjectId, params)  # check the status on the object
    ...
    time.sleep(
        5)  # wait 5 seconds if the media object is still being processed

publishImageResponse = publishMedia(imageMediaObjectId,
                                    params)  # publish the post to instagram
...
params[
    'media_url'] = 'https://justinstolpe.com/sandbox/ig_publish_content_vid.mp4'  # url on public server for the post
params[
...
while videoMediaStatusCode != 'FINISHED':  # keep checking until the object status is finished
    videoMediaObjectStatusResponse = getMediaObjectStatus(
        videoMediaObjectId, params)  # check the status on the object
...
publishVideoResponse = publishMedia(videoMediaObjectId,
                                    params)  # publish the post to instagram
...
contentPublishingApiLimit = getContentPublishingLimit(
    params)  # get the users api limit

print("\n---- CONTENT PUBLISHING USER API LIMIT -----\n")  # title
print("\tResponse:")  # label
print(
    contentPublishingApiLimit['json_data_pretty'])  # json response from ig api


https://api.instagram.com/oauth/authorize
  ?client_id=1140136490261427
